function [i] = hypsometry(raster, classes, options ,Sa, Sb, figHandle, totalDraws, ithDraw)
...
%        [figHandle] (optional) is a figure handle used to set a specific
%        figure window.
% -------------------------------------------------------------------------
% OUTPUTS:
...
% check if a specific figure handle is used
...
if options(2)==1
    ...
    ithColor = cMap(round(endColor*(ithDraw/totalDraws)),:); % plot 색 선정
    
    sColor = ['''Color''',',','[',num2str(ithColor),']'];
    
    figure(figHandle);
    
    eval(['plot(linspace(0,100,numel(raster)),raster,',Sa,',',pa,',',sColor,');']);    
