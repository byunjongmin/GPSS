...
{    
    ...
    for (ithCell=0;ithCell<consideringCellsNo;ithCell++)
    {
        ...
        /* 1) i번째 셀이 flooded region 유출구가 아니라면, 무한 유향을 따라 다음
         *    셀에 분배함
        ...
        if ((int) floodedRegionCellsNo[ithCellIdx] == 0)
        {
            ...
            /* (2) 유량과 누적 셀 개수를 유향을 따라 다음 셀들(e1,e2)에 분배함 */

            /* A. e1,e2의 유입량에 각각 전해질 유량과 누적 셀 개수를 구함 */
            outputDischargeToE1
            ...
            /* A) e1이 flooded region에 해당하는지 확인함 */
            if ( (int) flood[e1] == FLOODED )
            {

                /* (A) e1이 flooded region에 해당한다면 유량과 누적 셀 개수를
            ...
            {
                /* (B) e1이 flooded region이 아니라면, e1의 유입량에 상부 
            ...
            /* B) e2가 flooded region에 해당하는지 확인함 */
            if ( (int) flood[e2] == FLOODED )
            {

                /* (A) e2가 flooded region에 해당한다면 유량과 누적 셀 개수를 
                 *     flooded region의 유입량으로 반영함
                 * * 주의: 유량과 셀 개수의 처리 방식이 서로 다름
                 *   유량의 경우 flooded region의 저수량 초과분이 유출구에 전해지기
                 *   때문에 유출구의 유입량에 바로 더하지 않음 */

                /* a. flooded region의 유출구의 좌표를 파악함 */
                /* * 주의: MATLAB 배열 색인을 위해 '-1'을 수행함 */
            ...
            {
                /* (B) e2가 flooded region이 아니라면, e2의 유입량에 상부
                 *     유역으로부터의 유량과 누적 셀 개수를 더함 */
        ...
        /* (2) i번째 셀이 flooded region의 유출구라면 무한 유향을 이용하지 않고
         *     최대 하부 경사 유향 알고리듬을 이용한다. 즉, SDSNbrIndicies가 
         *     가리키는 다음 셀로 상부 유역의 유량과 셀의 갯수를 분배함
         *     이는 ProcessSink 함수에서 유출구의 유향이 flooded region을 다시
         *     향하지 않도록 SDSNbrY,SDSNbrX에 수정을 가했기 때문임 */
        else
        {
             ...
             * A) 다음 셀 색인
             * * 주의: 무한 유향을 이용하지 않고, SDSNbrIndicies를 따라 다음 셀에
             *   분배함 */
            /* * 주의: MATLAB 배열 색인을 위해 '-1'을 수행함 */
